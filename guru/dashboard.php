...
// Total murid dihitung HANYA untuk kelas yang diajar guru ini (sesuai kelas saat registrasi),
// bukan seluruh murid dari kelas 1-6.
$kelas_guru    = $_SESSION['kelas'] ?? '-';
$total_murid   = mysqli_fetch_row(mysqli_query($koneksi, "SELECT COUNT(*) FROM users WHERE role='murid' AND kelas='" . mysqli_real_escape_string($koneksi, $kelas_guru) . "'"))[0];
    ...
    <main class="max-w-5xl mx-auto px-4 mt-6 md:mt-8 space-y-6 md:space-y-8">
        ...
        <section class="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
            <div class="bg-blue-100 border-4 border-[#0f172a] p-4 md:p-5 rounded-2xl shadow-[4px_4px_0px_#0f172a] flex flex-col justify-between">
                <span class="text-[9px] md:text-[10px] font-900 uppercase tracking-widest text-[#0f172a]">Total Murid <?= $kelas_guru !== '-' ? '(Kelas ' . htmlspecialchars($kelas_guru) . ')' : ''; ?></span>
